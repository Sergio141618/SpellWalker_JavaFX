...
public class ConexionApi {
  ...
  public static int extraerNumero(String json) {
    try {
      ...
      if (valueIndex != -1) {
        int startIndex = valueIndex + 8;
        int endIndex = json.indexOf("}", startIndex);
        if (endIndex != -1) {
          String numberStr = json.substring(startIndex, endIndex).replaceAll("\"", "").strip();
          if (numberStr.equals("null")) {
            return 1; // Si la tabla está vacía, MAX+1 o COUNT devuelve null
          }
  ...
  public static java.util.List<String> obtenerTodasLasCampanas() throws IOException {
    String payload = """
        {
          "requests": [
            {
  ...
  private static java.util.List<String> extraerNombresDeJson(String json) {
